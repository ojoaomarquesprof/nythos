-- ============================================================
-- Migration: recomplete_reversed_sessions_billing
-- Purpose:
--   Allow safely completing a reverted single session again while
--   preserving cancelled cash_flow history.
-- ============================================================

DROP INDEX IF EXISTS public.cash_flow_session_id_uidx;
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'duplicate_active_cash_flow_session_id' USING ERRCODE = '23505';
  END IF;
END;
$$;

CREATE UNIQUE INDEX cash_flow_session_id_uidx
