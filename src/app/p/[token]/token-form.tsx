...
import { Heart, Sparkles, CalendarDays, ArrowRight, ShieldCheck, Loader2, AlertCircle } from "lucide-react";
...
export function PatientTokenForm({ token, firstName }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      // redirect() no servidor lança NEXT_REDIRECT — se chegarmos aqui é erro
      if (!result.success) setError(result.error ?? "Erro ao verificar. Tente novamente.");
    });
  }
            ...
            {firstName ? (
              <>
                ...
                <p className="text-sm text-[oklch(0.5_0.02_280)] mt-1 text-center">
                  Para acessar sua área, confirme sua data de nascimento.
                </p>
              </>
            ) : (
              <>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                required
                className="w-full px-5 py-3.5 rounded-2xl bg-white/70 border border-[oklch(0.92_0.01_290)] text-[oklch(0.22_0.02_280)] text-sm font-semibold focus:outline-none focus:border-[oklch(0.55_0.2_280)] focus:ring-4 focus:ring-[oklch(0.55_0.2_280)]/10 transition-all duration-200"
              />
            </div>

            {error && (
              <div role="alert" className="flex items-start gap-2.5 p-3.5 rounded-2xl bg-rose-50/80 border border-rose-200/60 text-rose-700 text-sm">
